"""Build the fresh, outcome-unopened C0 v2.2 pool after v2.1 capacity failure."""
...
POOL_SEED = 2026080317
CANARY_SEED = 2026080318
ROLLOUT_SEEDS = {
    "calibration": list(range(14001, 14033)),
    "qualification": list(range(15001, 15033)),
    "capacity_canary": list(range(13001, 13009)),
}


def _load_generator():
    spec = importlib.util.spec_from_file_location("rist_c0_v2_2_generator", GENERATOR_PATH)
...
def build_rows() -> dict[str, list[dict[str, Any]]]:
    ...
    canary = generator._build_task(
        "c0v2_2_capacity_canary", hardest, 0, CANARY_SEED
    )
    canary["id"] = "capacity-canary-v2_2-c07"
    ...
    if len(signatures) != len(set(signatures)):
        raise ValueError("C0 v2.2 task signatures overlap internally")
    if set(signatures) & _prior_signatures():
        raise ValueError("C0 v2.2 task signatures overlap a retired or downstream pool")
...
def write_pool(output_dir: Path) -> dict[str, Any]:
    ...
    manifest = {
        "protocol": "RIST-C0-v2.2-FRESH-POOL",
        "pool_seed": POOL_SEED,
        "capacity_canary_seed": CANARY_SEED,
        "previous_c0_task_pool_retired": True,
