"""Pure C0 v2.2 resolution, transport, and structural filtering logic."""
...
def calibrate_family(
    ...
    return {
        "protocol": "RIST-C0-v2.2-RESOLUTION-FAMILY-v1",
        "family": family,
...
def combine_families(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not all(row.get("passed") is True for row in results):
        return {
            "protocol": "RIST-C0-v2.2-CROSS-FAMILY-v1",
            "passed": False,
    ...
    return {
        ...
        "band_cell_counts": counts,
        "passed": passed,
        "decision": "PASS_C0_V2_2_TO_FILTERED_TRAIN" if passed else "KILL_C0_NO_COMMON_BANDS",
    }
