"""Finalize one two-family C0 v2.3 split without interpreting outcomes."""
...
def _load_validator():
    spec = importlib.util.spec_from_file_location("rist_c0_v2_3_job_validator", VALIDATOR_PATH)
    module = importlib.util.module_from_spec(spec)
    if spec.loader is None:
        raise RuntimeError("C0 v2.3 job validator is unavailable")
...
def finalize(manifest_path: Path) -> dict[str, Any]:
    ...
    if split not in {"calibration", "qualification"}:
        raise ValueError("C0 v2.3 finalization requires one frozen split")
    if manifest.get("trajectory_count") != 2048 or manifest.get("job_count") != 2:
        raise ValueError("C0 v2.3 finalization requires two jobs and 2,048 trajectories")
    jobs = manifest.get("jobs", [])
    expected_job_ids = {f"qwen3-{split}", f"gemma4-{split}"}
    if {str(job.get("job_id")) for job in jobs} != expected_job_ids:
        raise ValueError("C0 v2.3 finalization requires both families for one split")
    gate = manifest.get("canary_gate")
    if not isinstance(gate, dict) or gate.get("passed") is not True:
        raise PermissionError("C0 v2.3 finalization is blocked without canary PASS")
    ...
    return {
        "protocol": "RIST-C0-v2.3-SCIENTIFIC-COLLECTION-FINAL-v1",
        ...
        "passed": passed,
        "decision": (
            f"PASS_{split.upper()}_COLLECTION_TO_SEPARATE_ANALYSIS"
            if passed
            else f"KILL_C0_V2_3_{split.upper()}_POOL"
        ),
    }
