...
def run_canary(
    ...
    if len(tasks) != 1 or len(seeds) != 8 or len(seeds) != len(set(seeds)):
        raise ValueError("capacity canary requires one task and eight unique frozen requests")
...
def main() -> int:
    ...
    result = run_canary(
        json.loads(args.pool_manifest.read_text(encoding="utf-8")),
        args.data_dir,
        args.family,
