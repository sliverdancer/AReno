"""Collect one RIST C0 v3 scientific job through the v3 request contract.

This module is self-contained within the v3 namespace. It intentionally does
...
PROTOCOL = "RIST-C0-v3.0-SCIENTIFIC-JOB-EVIDENCE-v1"
MANIFEST_PROTOCOL = "RIST-C0-v3.0-STAGE-MANIFEST-v1"
POOL_PROTOCOL = "RIST-C0-v3.0-FRESH-POOL-v1"
SYSTEM_PROMPT = (
...
def _validate_gate(manifest: dict[str, Any], job: dict[str, Any], identity: dict[str, Any]) -> None:
    ...
    if manifest.get("split") not in {"calibration", "qualification"}:
        raise ValueError("v3 collection requires calibration or qualification")
    if manifest.get("retry_permitted") is not False or job.get("max_retries") != 0:
        raise PermissionError("v3 scientific collection forbids retries")
    if any(
        ...
        raise PermissionError("v3 scientific collection crosses a forbidden boundary")
    if manifest["split"] == "calibration" and not (
        ...
        raise PermissionError("invalid v3 calibration boundary")
    if manifest["split"] == "qualification" and not (
        ...
        raise PermissionError("invalid v3 qualification boundary")
...
def collect_job(
    *,
    ...
    if pool_manifest.get("protocol") != POOL_PROTOCOL:
        raise ValueError("unexpected RIST C0 v3 pool manifest")
    if any(path.exists() for path in (result_path, trajectory_path, journal_path)):
        raise FileExistsError("v3 scientific evidence paths must be fresh")

    task_path = Path(job["task_file"])
    if _sha256(task_path) != job["task_file_sha256"]:
        raise ValueError("v3 scientific split hash mismatch")
    ...
    if expected_count != int(job["trajectory_count"]):
        raise ValueError("v3 scientific job trajectory count mismatch")
    if expected_count > 1024:
        raise ValueError("v3 scientific job exceeds one family split")
    concurrency = int(job["concurrency"])
    if concurrency <= 0 or concurrency != int(manifest.get("request_concurrency", concurrency)):
        raise ValueError("v3 request concurrency mismatch")
    sampling = manifest.get("sampling")
    if not isinstance(sampling, dict):
        raise ValueError("v3 sampling contract is missing")
...
def main() -> int:
    ...
    if _sha256(pool_path) != manifest["pool_manifest_sha256"]:
        raise ValueError("v3 pool manifest hash mismatch")
    result = collect_job(
        manifest=manifest,
