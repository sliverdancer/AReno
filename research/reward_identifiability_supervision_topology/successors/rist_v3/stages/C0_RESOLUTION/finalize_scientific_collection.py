"""Finalize one two-family RIST C0 v3 split without interpreting outcomes."""
...
def _load_validator():
    spec = importlib.util.spec_from_file_location("rist_c0_v3_job_validator", VALIDATOR_PATH)
    module = importlib.util.module_from_spec(spec)
    if spec.loader is None:
        raise RuntimeError("RIST C0 v3 job validator is unavailable")
...
def finalize(manifest_path: Path) -> dict[str, Any]:
    ...
    if split not in {"calibration", "qualification"}:
        raise ValueError("v3 finalization requires one frozen split")
    jobs = manifest.get("jobs", [])
    expected_job_ids = {f"qwen3-{split}", f"gemma4-{split}"}
    if {str(job.get("job_id")) for job in jobs} != expected_job_ids:
        raise ValueError("v3 finalization requires both families for one split")
    if int(manifest.get("job_count", -1)) != 2:
        raise ValueError("v3 finalization requires two jobs")
    if any(
        ...
        raise PermissionError("v3 finalization crosses a forbidden boundary")
    ...
    for job in jobs:
        ...
        if validation["passed"] is not True:
            raise ValueError(f"v3 scientific job validation failed: {job['job_id']}")
        rows.append(
            {
                "job_id": job["job_id"],
                "family": job["family"],
    ...
    return {
        "protocol": "RIST-C0-v3.0-SCIENTIFIC-COLLECTION-FINAL-v1",
        ...
        "decision": (
            ...
            if passed
            else f"KILL_C0_V3_{str(split).upper()}_POOL"
        ),
    }
