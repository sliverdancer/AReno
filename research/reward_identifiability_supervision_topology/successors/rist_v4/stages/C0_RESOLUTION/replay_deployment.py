"""Execute the exact v4 child-launch gate without a model, server, request, or GPU."""
...
def replay(mode: str, output: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="rist-v4-replay-") as raw_root:
        root = Path(raw_root)
        source = root / "source"
        ...
        manifest.write_text('{"protocol":"RIST-C0-v4.0-REPLAY"}\n', encoding="utf-8")
        ...
        result = {
            "protocol": "RIST-C0-v4.0-EXACT-DEPLOYMENT-REPLAY-v1",
            "mode": mode,
