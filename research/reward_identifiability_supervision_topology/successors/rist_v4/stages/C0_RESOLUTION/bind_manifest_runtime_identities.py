"""Bind non-circular live runtime identities into a RIST C0 v4 manifest.
...
def bind_manifest(
    *,
    ...
    if manifest.get("protocol") != "RIST-C0-v4.0-STAGE-MANIFEST-v1":
        raise ValueError("unexpected RIST C0 v4 stage manifest")
    ...
    for job in value.get("jobs", []):
        ...
        if family not in model_paths:
            raise ValueError(f"unexpected RIST v4 family: {family}")
