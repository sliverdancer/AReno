"""Sole interpreter-bound, fail-closed deployment gate for RIST C0 v4."""
...
PROTOCOL = "RIST-C0-v4.0-DEPLOYMENT-RECEIPT-v1"
AUTHORITY_PROTOCOL = "RIST-C0-v4.0-DEPLOYMENT-AUTHORITY-v1"
FIELDS = (
    "control_commit",
    "runtime_commit",
    "manifest_sha256",
...
def _unpack(receipt: Mapping[str, Any]) -> tuple[dict[str, str], bytes, list[str]]:
    ...
    if not isinstance(bindings, dict) or set(bindings) != set(FIELDS):
        raise DeploymentRefused("receipt must bind exactly the v4 identities")
    try:
