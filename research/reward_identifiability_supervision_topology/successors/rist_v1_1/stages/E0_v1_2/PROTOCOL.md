...
`RIST-E0-v1.1` then terminated at preflight without starting a server because
its frozen command named a nonexistent top-level Python module. The compiled
extension and `areno check` both passed under the registered module path. This
revision changes only that preflight symbol and the protocol namespace; it does
not repair, reinterpret, or splice the terminal v1.1 outcome.
...
1. `python -c "import areno.accel._areno_accel"` and assert that the imported
   shared object resides under the frozen editable source checkout;
...
The prior v1.1 authorization does not carry over. Even though v1.1 used zero
serving seconds, v1.2 requires a new explicit GPU-serving authorization.
