...
def collect(
    *,
    ...
    if protocol.get("protocol") != "RIST-P2.1-v1.0":
        raise ValueError("unexpected P2.1 protocol")
    tasks = [
        json.loads(line)
    ...
    if len(tasks) != int(protocol["tasks_per_model"]):
        raise ValueError("qualification task count does not match frozen P2.1 manifest")
    if any(len(task.get("turns", [])) != int(protocol["turns_per_trajectory"]) for task in tasks):
        raise ValueError("qualification turn count does not match frozen P2.1 manifest")
    ...
    result = {
        ...
        "fabricated_call_count": sum(
            int(trajectory.get("fabricated_call_count", 0))
            for trajectory in trajectories
        ),
        "retry_count": 0,
        "qualification_data_opened": True,
