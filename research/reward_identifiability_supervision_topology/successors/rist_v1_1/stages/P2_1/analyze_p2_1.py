...
def analyze_model(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    evidence_complete = (
        ...
        and all(len(row.get("raw_responses", [])) == 4 for row in trajectories)
    )
    ...
    gates = {
        ...
        and fabricated == 0
        and int(payload.get("retry_count", -1)) == 0
...
def analyze_cross_family(payloads: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if any(payload.get("protocol") != "RIST-P2.1-v1.0" for payload in payloads):
        raise ValueError("unexpected P2.1 result protocol")
    qualification_hashes = {payload.get("qualification_sha256") for payload in payloads}
    if qualification_hashes != {
        "8018137606e12da0f0096ac86f11312d94d631326965198783ebf9cecc94570f"
    }:
        raise ValueError("P2.1 results do not share the frozen qualification hash")
    ...
    if any(payload.get("infrastructure_error") is not None for payload in payloads):
        decision = "INVALID_P2_1_PREFLIGHT_OR_INFRASTRUCTURE"
        status = "INVALID"
    elif not all(
        model["gates"]["first_turn_executable"]
        and model["gates"]["four_turn_complete"]
        for model in models
    ):
        decision = "KILL_P2_1_CHECKPOINT_INTERFACE"
        status = "KILL"
    elif not passed:
        decision = "KILL_P2_1_TASK_INSTRUMENT_NO_MODEL_RESOLUTION"
        status = "KILL"
    else:
        decision = "PASS_P2_1_CROSS_FAMILY_RESOLUTION_TO_P3_PROTOCOL_FREEZE"
        status = "PASS"
    return {
        "schema_version": 1,
